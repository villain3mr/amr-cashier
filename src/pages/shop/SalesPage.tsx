...
import { Plus, Minus, Trash2, Printer, Search, ArrowDownUp } from 'lucide-react';

const SalesPage: React.FC = () => {
  const { auth, products, customers, addInvoice, settings } = useApp();
  ...
  const activePaymentMethods = useMemo(() => settings.paymentMethods.filter(m => m.active), [settings.paymentMethods]);

  const [invoiceType, setInvoiceType] = useState<'sale' | 'purchase'>('sale');
  const [items, setItems] = useState<InvoiceItem[]>([]);
  const [discount, setDiscount] = useState(0);
  const [paid, setPaid] = useState(0);
  const [paymentMethod, setPaymentMethod] = useState(activePaymentMethods[0]?.id || 'cash');
  ...
  const addItem = (productId: string) => {
    ...
    if (existing) {
      if (invoiceType === 'sale' && existing.quantity >= product.quantity) return;
      setItems(items.map(i => i.productId === productId
        ? { ...i, quantity: i.quantity + 1, total: (i.quantity + 1) * i.unitPrice }
        : i
      ));
    } else {
      const price = invoiceType === 'sale' ? product.sellPrice : product.buyPrice;
  ...
  const updateQuantity = (itemId: string, delta: number) => {
    setItems(items.map(i => {
      ...
      const maxQty = invoiceType === 'sale' ? (product?.quantity || 1) : 9999;
  ...
  const handleSave = () => {
    ...
    const methodLabel = activePaymentMethods.find(m => m.id === paymentMethod)?.label || paymentMethod;
    addInvoice({
      shopId: auth.shopId!,
    ...
    <div className="h-full flex">
        ...
        <div className="p-4 border-b border-border space-y-2">
          ...
          <div className="flex border border-border rounded-md overflow-hidden">
            <button
              ...
              className={`flex-1 py-2 text-xs font-body transition-colors flex items-center justify-center gap-1 ${
                invoiceType === 'sale' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'
              }`}
            >
              بيع
            </button>
            <button
              onClick={() => { setInvoiceType('purchase'); setItems([]); }}
              className={`flex-1 py-2 text-xs font-body transition-colors flex items-center justify-center gap-1 ${
                invoiceType === 'purchase' ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'
              }`}
            >
              <ArrowDownUp className="w-3 h-3" strokeWidth={1.5} />
              شراء من عميل
            </button>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {filteredProducts.map(product => (
                <button
                  ...
                  disabled={invoiceType === 'sale' && product.quantity <= 0}
                ...
                >
                  ...
                  <p className="font-body text-lg font-medium text-primary mt-1">
                    {(invoiceType === 'sale' ? product.sellPrice : product.buyPrice).toLocaleString()} {settings.currency}
                  </p>
      ...
      <div className="w-72 lg:w-80 bg-card border-r border-border h-full flex flex-col overflow-auto p-4">
        <h2 className="font-heading font-semibold text-foreground mb-4">
          {invoiceType === 'sale' ? 'ملخص فاتورة البيع' : 'ملخص فاتورة الشراء'}
        </h2>
            ...
            {activePaymentMethods.map(method => (
              <button
                ...
                className={`flex-1 py-2 text-xs font-body transition-colors ${
                  paymentMethod === method.id ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:text-foreground'
                }`}
              >
                {method.label}
              </button>
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between text-sm font-body">
            ...
            <span className="text-foreground font-medium">{subtotal.toLocaleString()} {settings.currency}</span>
          </div>
          ...
          {remaining > 0 && (
            <div className="flex justify-between text-sm font-body">
              ...
              <span className="text-destructive font-medium">{remaining.toLocaleString()} {settings.currency}</span>
            </div>
          )}
        </div>
